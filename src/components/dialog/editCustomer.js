...
function EditCustomer(props) {
  const { updateCustomer, fetchCustomers } = useCustomer();

  const [name, setName] = useState(props.client.name);
  const [address, setAddress] = useState(props.client.address);
  const [recepient, setRecepient] = useState(props.client.recepient);
  const [contact, setContact] = useState(props.client.contact);

  const handleSubmit = (e) => {
    e.preventDefault();
    const payload = { _id: props.client._id, name, contact, address, recepient };
